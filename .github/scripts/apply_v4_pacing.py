...
// ── V4 · PACING POLISH ─────────────────────────────────────
// La dirección no solo decide eventos: da a cada wave una intención perceptible.
// El jugador recibe contexto y una pista de que la señal puede romperse, pero no
// conoce el contenido exacto hasta el telegraph. Esto refuerza anticipación sin
// convertir la partida en un guion rígido.
const V4_PACING={
...
function showPacingBeat(){
  ...
  if(!beat) return;
  // Solo las waves de señal fuerte interrumpen visualmente; las demás viven en HUD.
  if(beat.tone==='signal'||beat.tone==='hot'){
...
function updateDirectorHud(){
  ...
  if(a&&a.phase==='armed'){
    const left=Math.max(0,a.timer/60);
    el.textContent='SIGNAL // ?  '+(left<1.1?'IMMINENTE':'DETECTADA');
    el.className='signal';
    return;
  }
  const beat=pacingBeatForWave();
...
function drawDirectorAtmosphere(){
  const d=eventDirector;
  if(!d) return;
  // Mientras un evento está armado, el jugador ve que la partida está cambiando
  // sin saber aún qué. El telegraph detallado sigue perteneciendo al evento.
  if(d.active&&d.active.phase==='armed'){
    const p=0.18+0.12*Math.sin(frame*0.11);
    cx.save();
    cx.globalAlpha=reducedMotion?0.12:p;
    cx.strokeStyle='#ff6';
    cx.lineWidth=1;
    cx.beginPath();
    cx.moveTo(W*0.35,7); cx.lineTo(W*0.65,7);
    cx.stroke();
    cx.restore();
  }
}
...
# Make event-bearing wave slots feel intentionally authored, while calm waves stay calm.
...
assert old in s, 'event chance line missing'
s = s.replace(old, new, 1)

# Make the route more discoverable without making it automatic.
old = 'const bySecret=d.routeFlags.transmissions>=3;'
new = 'const bySecret=d.routeFlags.transmissions>=2;'
assert old in s, 'route secret threshold missing'
s = s.replace(old, new, 1)
...
assert old in s, 'route skill threshold missing'
s = s.replace(old, new, 1)

# Arm state exposes a hidden forecast without naming the event.
...
assert old in s, 'arm state line missing'
s = s.replace(old, new, 1)

# Hide forecast the moment the explicit telegraph begins; the event itself takes over.
old = "  a.telegraphed=true;\n"
new = "  a.telegraphed=true;\n  if(eventDirector) eventDirector.forecast=null;\n"
assert old in s, 'telegraph marker missing'
s = s.replace(old, new, 1)

# Resolve clears forecast and keeps a memory for the next pacing beat.
old = "  d.active=null;\n  if(V4_DEBUG) console.debug('[V4 EVENT] resolve'"
new = "  d.active=null; d.forecast=null; d.lastOutcome=outcome;\n  if(V4_DEBUG) console.debug('[V4 EVENT] resolve'"
assert old in s, 'resolve anchor missing'
s = s.replace(old, new, 1)

# Every frame: detect a new wave once, refresh HUD cheaply, and draw atmosphere.
old = "  if(!d||state!=='playing') return;\n  d.sampleAcc+=dt;"
new = "  if(!d||state!=='playing') return;\n  showPacingBeat();\n  updateDirectorHud();\n  d.sampleAcc+=dt;"
assert old in s, 'director tick anchor missing'
s = s.replace(old, new, 1)
...
assert old in s, 'render anchor missing'
s = s.replace(old, new, 1)

# Keep HUD text consistent after a route change / wave transition.
old = "  if(!d||d.route!=='alpha') return;\n  const bySecret=d.routeFlags.transmissions>=2;"
new = "  if(!d||d.route!=='alpha') return;\n  const bySecret=d.routeFlags.transmissions>=2;"
# no-op anchor assertion
assert old in s, 'route block missing after threshold patch'

# Update docs marker/version note without changing the public version.
